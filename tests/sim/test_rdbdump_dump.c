...
static int generate_kvdb(const char* out_dir) {
    ...
    if (sim_flash_init(&flash, g_flash_buf, FLASH_SIZE, SECTOR_SIZE, PAGE_SIZE, 0) != 0)
        return -1;
    db.part       = &part;
    db.sectors    = meta;
    ...
    if (rdb_kvdb_format(&db) != RDB_OK)
        return -1;
    if (rdb_kvdb_init(&db, &part, meta) != RDB_OK)
        return -1;
    if (rdb_kvdb_set(&db, "wifi", (const uint8_t*)"lab-net", 7) != RDB_OK)
        return -1;
    if (rdb_kvdb_set(&db, "mode", (const uint8_t*)"normal", 6) != RDB_OK)
        return -1;
    if (rdb_kvdb_set(&db, "mode", (const uint8_t*)"service", 7) != RDB_OK)
        return -1;
    if (rdb_kvdb_delete(&db, "wifi") != RDB_OK)
        return -1;
    if (rdb_kvdb_set(&db, "count", count, sizeof(count)) != RDB_OK)
        return -1;
    ...
    if (sim_flash_save_file(&flash, image) != 0)
        return -1;
    if (write_manifest(manifest, "kvdb", "rdbdump_kvdb.bin") != 0)
        return -1;
    return 0;
}
