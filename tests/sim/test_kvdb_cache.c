...
static int kv_read(void* ctx, uint32_t addr, uint8_t* buf, size_t len) {
    (void)ctx;
    return sim_flash_read(&g_kv_sim, addr, buf, len);
}
static int kv_write(void* ctx, uint32_t addr, const uint8_t* buf, size_t len) {
    (void)ctx;
    return sim_flash_write(&g_kv_sim, addr, buf, len);
}
static int kv_erase(void* ctx, uint32_t addr) {
    (void)ctx;
    return sim_flash_erase(&g_kv_sim, addr);
}
...
static int kv_setup(void) {
    if (sim_flash_init(&g_kv_sim, g_kv_flash, KV_FLASH_SIZE, KV_SECTOR_SIZE, KV_PAGE_SIZE, KV_WG) != 0)
        return -1;
    sim_flash_set_trace(&g_kv_sim, &g_trace);
    g_kv_part = (rdb_partition_t) {.name = "KVDB",
        .base_addr                       = 0,
        .total_size                      = KVDB_PART_SIZE,
        .sector_size                     = KV_SECTOR_SIZE,
        .write_gran                      = KV_WG,
        .ops                             = &g_kv_ops};
    ...
    trace_event(&g_trace, "KVDB format+init (64 sectors, %u KB)", (unsigned)(KVDB_PART_SIZE / 1024u));
    ...
    if (rc == RDB_OK) {
        ...
        trace_event(&g_trace, "  [CACHE] %u slots (%u bytes) enabled", (unsigned)RDB_KV_CACHE_SIZE,
...
static int ts_read(void* ctx, uint32_t addr, uint8_t* buf, size_t len) {
    (void)ctx;
    return sim_flash_read(&g_ts_sim, addr, buf, len);
}
static int ts_write(void* ctx, uint32_t addr, const uint8_t* buf, size_t len) {
    (void)ctx;
    return sim_flash_write(&g_ts_sim, addr, buf, len);
}
static int ts_erase(void* ctx, uint32_t addr) {
    (void)ctx;
    return sim_flash_erase(&g_ts_sim, addr);
}
...
static int ts_setup(void) {
    if (sim_flash_init(&g_ts_sim, g_ts_flash, TS_FLASH_SIZE, TS_SECTOR_SIZE, TS_PAGE_SIZE, TS_WG) != 0)
        return -1;
    sim_flash_set_trace(&g_ts_sim, &g_trace);
    g_ts_part = (rdb_partition_t) {.name = "TSDB",
        .base_addr                       = 0,
        .total_size                      = TSDB_PART_SIZE,
        .sector_size                     = TS_SECTOR_SIZE,
        .write_gran                      = TS_WG,
        .ops                             = &g_ts_ops};
    ...
    trace_event(&g_trace, "TSDB format+init (32 sectors, %u KB)", (unsigned)(TSDB_PART_SIZE / 1024u));
...
static rdb_err_t trace_kv_get(const char* key, void* buf, uint16_t buf_len, uint16_t* out_len) {
...
static int cache_used(void) {
    ...
    for (unsigned i = 0; i < RDB_KV_CACHE_SIZE; i++) {
        if (g_kv_db.cache.slots[i].klen != 0)
            n++;
    }
    return n;
}
...
static void trace_cache_stats(const char* label) {
    int used = cache_used();
    trace_event(&g_trace, "  [CACHE] %s — %d/%u slots used (%u%%)", label, used, (unsigned)RDB_KV_CACHE_SIZE,
...
static uint16_t make_value(int i, uint8_t* buf, size_t buf_sz) {
    ...
    if (cat == 0)
        ...
        vlen = (uint16_t)(8 + (i % 25));
    else if (cat == 2)
        vlen = (uint16_t)(64 + (i % 65));
    else if (cat == 3)
        vlen = (uint16_t)(200 + (i % 56));
    else
        vlen = (uint16_t)(250 + (i % 6));
    if (vlen > buf_sz)
        vlen = (uint16_t)buf_sz;
...
static int kv_cache_write_read_delete(void* ctx) {
    ...
    {
        char label[48];
        snprintf(label, sizeof(label), "after %d writes", N_KEYS);
    ...
    for (int i = 0; i < 40; i++) {
        char key[32];
        snprintf(key, sizeof(key), "k%03d", i);
        TEST_ASSERT_RDB_ERR(rdb_kvdb_get(&g_kv_db, key, NULL, 0, NULL), RDB_ERR_NOT_FOUND);
    }
...
static int kv_cache_hot_key_update(void* ctx) {
    ...
    {
        char label[48];
        snprintf(label, sizeof(label), "after %d hot-key updates", N_UPDATES);
        trace_cache_stats(label);
    }
...
static int kv_cache_gc_stress(void* ctx) {
    ...
    trace_event(&g_trace, "[kv-cache-gc-stress] GC stress: %d rounds x %d keys (varied sizes)", N_ROUNDS, N_KEYS);
    for (int round = 0; round < N_ROUNDS; round++) {
        ...
        {
            char label[48];
            snprintf(label, sizeof(label), "after round %d", round);
            trace_cache_stats(label);
        }
    }
    ...
    for (int i = 0; i < N_KEYS; i++) {
        ...
        uint8_t expected[BUF_SZ];
        snprintf(key, sizeof(key), "gk%03d", i);
        (void)make_value((N_ROUNDS - 1) * N_KEYS + i, expected, sizeof(expected));
        uint8_t   buf[BUF_SZ];
        uint16_t  olen = 0;
...
static int ts_count_cb(uint32_t time, const void* data, uint16_t len, void* arg) {
    (void)time;
    (void)data;
    (void)len;
    int* cnt = (int*)arg;
...
static int ts_append_query_ring(void* ctx) {
    ...
    trace_event(&g_trace, "  [ts-append-query] total_count=%u write_ops=%u head_sec=%u", (unsigned)g_ts_db.total_count,
        (unsigned)g_ts_db.stats.write_ops, (unsigned)g_ts_db.head_sec);

    TEST_ASSERT_LE(g_ts_db.total_count, g_ts_db.stats.write_ops);
    TEST_ASSERT_GT(g_ts_db.total_count, 0u);

    int count = 0;
    trace_event(&g_trace, "[ts-append-query] Full range query (1000..%u)", (unsigned)g_ts_db.last_time);
    TEST_ASSERT_RDB_OK(rdb_tsdb_query(&g_ts_db, 1000, g_ts_db.last_time, ts_count_cb, &count));
    ...
    TEST_ASSERT_RDB_OK(rdb_tsdb_query(&g_ts_db, 5000, 10000, ts_count_cb, &sub));
...
static int ts_safety_recover_faults(void* ctx) {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        TEST_ASSERT_RDB_OK(trace_ts_append((uint32_t)((i + 1) * 100), data, sizeof(data)));
    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        rdb_err_t rc = trace_ts_append((uint32_t)((i + 51) * 100), data, sizeof(data));
        if (rc == RDB_ERR_FLASH) {
    ...
    for (int i = 0; i < 30; i++) {
        ...
        TEST_ASSERT_RDB_OK(trace_ts_append((uint32_t)((i + 1) * 200), data, sizeof(data)));
        recovered_appends++;
    }
...
static int kv_cache_gc_migration(void* ctx) {
    ...
    {
        char label[48];
        snprintf(label, sizeof(label), "after phase 1 (%d keys)", N_KEYS);
        trace_cache_stats(label);
    }
...
static int kv_cache_collision_stress(void* ctx) {
    ...
    trace_event(&g_trace, "[kv-cache-collide] Cache collision: write %d keys (same prefix+len, varied vals)", N_KEYS);
    for (int i = 0; i < N_KEYS; i++) {
        char    key[32];
        uint8_t val[BUF_SZ];
        memset(key, 'X', 28);
    ...
    {
        char label[56];
        snprintf(label, sizeof(label), "after %d collision keys written", N_KEYS);
        trace_cache_stats(label);
    }

    trace_event(&g_trace, "[kv-cache-collide] First read: verify all %d under collision pressure", N_KEYS);
    for (int i = 0; i < N_KEYS; i++) {
        char    key[32];
        uint8_t expected[BUF_SZ];
        memset(key, 'X', 28);
...
static test_case_t g_cases[] = {
    ...
    {"kv_cache_hot_key_update", "KVDB cache: hot-key 500 updates (cycling sizes)", kv_cache_hot_key_update,
        "KVDB-Cache", 1, NULL},
    {"kv_cache_gc_stress", "KVDB GC: 120 keys x 5 rounds, varied-length stress", kv_cache_gc_stress, "KVDB-GC", 1, NULL},
    {"ts_append_query_ring", "TSDB: 2000 appends across 32-sector ring (1-32B data)", ts_append_query_ring,
        "TSDB-Recount", 1, NULL},
    {"init_format_large_sectors", "Large-sector init/format (KV 64 + TS 32)", init_format_large_sectors, "Init", 1, NULL},
    {"ts_safety_recover_faults", "TSDB safety: recover after write faults", ts_safety_recover_faults, "TSDB-Safety", 1,
        NULL},
    {"kv_cache_gc_migration", "KVDB cache: 100 keys GC migration, varied-length values", kv_cache_gc_migration,
        "KVDB-GC", 1, NULL},
#if RDB_KV_CACHE_SIZE > 0
    {"kv_cache_collision_stress", "KVDB cache: 100-key collision stress, varied values", kv_cache_collision_stress,
        "KVDB-Cache", 1, NULL},
#endif
    {"kv_cache_max_key_hit", "KVDB cache: cache-hit verification with 32-byte max key", kv_cache_max_key_hit,
        "KVDB-Cache", 1, NULL},
};
...
int main(int argc, char** argv) {
    ...
    test_config_t config = {.log_file = fopen(test_make_log_path("kvdb_cache"), "w"),
        .verbose                      = 1,
        .stop_on_fail                 = 0,
        .filter                       = NULL,
        .post_test_hook               = post_test_sectors,
        .hook_ctx                     = NULL};
    ...
    trace_event(&g_trace, "KVDB: %u sectors (%u KB)  TSDB: %u sectors (%u KB)", (unsigned)KV_SECTOR_CNT,
        (unsigned)(KVDB_PART_SIZE / 1024u), (unsigned)TS_SECTOR_CNT, (unsigned)(TSDB_PART_SIZE / 1024u));
#if RDB_KV_CACHE_SIZE > 0
    trace_event(
        &g_trace, "KV Cache: %u slots (%u bytes)", (unsigned)RDB_KV_CACHE_SIZE, (unsigned)sizeof(rdb_kv_cache_t));
#else
