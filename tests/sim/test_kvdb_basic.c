...
static rdb_err_t kv_reset(uint8_t write_gran) {
    if (sim_flash_init(&g_flash, g_flash_buf, FLASH_SIZE, SECTOR_SIZE, PAGE_SIZE, write_gran) != 0)
...
TEST_CASE(kv_write_gran_matrix, "KVDB", "Write granularity matrix 1/2/4/8B") {
    ...
    for (uint8_t gran = 0; gran <= 3; gran++) {
        ...
        out_len = 0;
        TEST_ASSERT_RDB_OK(rdb_kvdb_get(&g_db, "WG_LARGE", large_out, (uint16_t)sizeof(large_out), &out_len));
...
TEST_CASE(kv_dedup_fingerprint_collision, "KVDB", "Dedup keeps distinct keys with matching hash prefix") {
    (void)ctx;
    TEST_ASSERT_RDB_OK(kv_reset(DEFAULT_WG));
...
TEST_CASE(kv_mixed_lengths, "KVDB", "Mixed key/value length Monte Carlo") {
    ...
    for (uint32_t i = 0; i < MIX_KEY_CNT; i++) {
        ...
        for (uint32_t j = pl; j < klen; j++)
            keys[i][j] = (char)('a' + (i % 26u));
        keys[i][klen]   = '\0';
        expected_len[i] = 0;
    }
    ...
    for (uint32_t round = 0; round < MIX_ROUNDS; round++) {
        for (uint32_t i = 0; i < MIX_KEY_CNT; i++) {
            ...
            for (uint16_t j = 0; j < vlen; j++)
                expected[i][j] = (uint8_t)kv_rand_next(&seed);
            expected_len[i] = vlen;
...
TEST_CASE(kv_corrupt_header_skip, "KVDB", "Corrupt header skip during iteration") {
    ...
    {
        uint8_t z = 0x00;
        sim_flash_write(&g_flash, first_addr, &z, 1);
    }
...
TEST_CASE(kv_init_format_verify, "KVDB", "Empty init and format verification") {
    ...
    TEST_ASSERT_EQ(g_db.sectors[g_db.active_sec].write_off, (uint16_t)kv_data_start(DEFAULT_WG));
    ...
    for (uint8_t s = 0; s < KV_SECTOR_CNT; s++) {
        if (g_db.sectors[s].status == RDB_SEC_ACTIVE || g_db.sectors[s].status == RDB_SEC_SEALED)
...
TEST_CASE(kv_max_boundaries, "KVDB", "Maximum key/value boundary test") {
    ...
    /* ── Max value length (computed from geometry) ── */
    {
        ...
        TEST_ASSERT_RDB_ERR(trace_kv_set(&g_db, "OV", mv, (uint16_t)(max_val + 1u)), RDB_ERR_TOO_LARGE);
        free(mv);
    }
    ...
    /* Value far too large (4096) → TOO_LARGE for any configuration */
    {
        ...
        TEST_ASSERT(ov != NULL);
        TEST_ASSERT_RDB_ERR(trace_kv_set(&g_db, "FV", ov, RDB_MAX_VAL_LEN + 1u), RDB_ERR_TOO_LARGE);
        free(ov);
    }
...
int main(void) {
    test_config_t config = {.log_file = fopen(test_make_log_path("kvdb_basic"), "w"),
        .verbose                      = 1,
        .stop_on_fail                 = 0,
        .filter                       = NULL,
