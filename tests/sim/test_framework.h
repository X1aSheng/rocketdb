 ...
 * @brief 断言条件为真
 */
#define TEST_ASSERT(cond)              TEST_ASSERT_IMPL((cond), #cond)

/**
 * @brief 断言条件为真，带自定义消息
 */
#define TEST_ASSERT_MSG(cond, msg)     TEST_ASSERT_IMPL((cond), msg)

/**
 * @brief 断言两个整数相等
 */
#define TEST_ASSERT_EQ(a, b)           TEST_ASSERT_IMPL((a) == (b), #a " == " #b)

/**
 * @brief 断言两个整数不相等
 */
#define TEST_ASSERT_NE(a, b)           TEST_ASSERT_IMPL((a) != (b), #a " != " #b)

/**
 * @brief 断言 a < b
 */
#define TEST_ASSERT_LT(a, b)           TEST_ASSERT_IMPL((a) < (b), #a " < " #b)

/**
 * @brief 断言 a <= b
 */
#define TEST_ASSERT_LE(a, b)           TEST_ASSERT_IMPL((a) <= (b), #a " <= " #b)

/**
 * @brief 断言 a > b
 */
#define TEST_ASSERT_GT(a, b)           TEST_ASSERT_IMPL((a) > (b), #a " > " #b)

/**
 * @brief 断言 a >= b
 */
#define TEST_ASSERT_GE(a, b)           TEST_ASSERT_IMPL((a) >= (b), #a " >= " #b)

/**
 * @brief 断言指针为 NULL
 */
#define TEST_ASSERT_NULL(ptr)          TEST_ASSERT_IMPL((ptr) == NULL, #ptr " is NULL")

/**
 * @brief 断言指针不为 NULL
 */
#define TEST_ASSERT_NOT_NULL(ptr)      TEST_ASSERT_IMPL((ptr) != NULL, #ptr " is not NULL")

/**
 * @brief 断言内存块相等
 */
#define TEST_ASSERT_MEM_EQ(a, b, len)  TEST_ASSERT_IMPL(memcmp((a), (b), (len)) == 0, "memory blocks equal")

/**
 * @brief 断言字符串相等
 */
#define TEST_ASSERT_STR_EQ(a, b)       TEST_ASSERT_IMPL(strcmp((a), (b)) == 0, "strings equal")

/**
 * @brief 断言 RocketDB 错误码为 RDB_OK
 */
#define TEST_ASSERT_RDB_OK(expr)       TEST_ASSERT_IMPL((expr) == RDB_OK, #expr " == RDB_OK")

/**
 * @brief 断言 RocketDB 错误码为指定值
 */
#define TEST_ASSERT_RDB_ERR(expr, err) TEST_ASSERT_IMPL((expr) == (err), #expr " == " #err)
...
#define TEST_CASE(fn_name, category, description)                                                                 \
    ...
    static test_case_t test_case_##fn_name = {#fn_name, (description), test_func_##fn_name, (category), 1, NULL}; \
    static int         test_func_##fn_name(void* ctx)
...
#define REGISTER_TEST(fn_name)                                              \
    __attribute__((constructor)) static void register_##fn_name(void) {     \
 ...
 * @return      静态缓冲区指针，格式 "tests/out/YYMMDD-HHMMSS-name.log"
 *
 * 调用者不应释放返回的指针。每次调用会覆盖上一次的结果。
 * 如果 tests/out/ 目录不存在，会自动创建。
 */
