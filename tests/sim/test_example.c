...
int main(void)
{
    ...
    g_part = (rdb_partition_t) {
        .name = "example", .base_addr = 0, .total_size = EX_FLASH_SIZE,
        .sector_size = EX_SECTOR_SIZE, .write_gran = 0, .ops = &g_ops
    };
    ...
    test_run_parameterized("example_param_write_gran", 
                          example_param_write_gran_test,
                          &env, params, 4);
