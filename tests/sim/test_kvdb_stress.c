...
static rdb_err_t kv_reset(void) {
    ...
    g_part          = (rdb_partition_t) {.name = "KVDB",
                 .base_addr                    = 0,
...
static int key_index(const char* key) {
    if (!key || key[0] != 'K' || key[1] < '0' || key[1] > '9' || key[2] < '0' || key[2] > '9')
        return -1;
...
TEST_CASE(kv_gc_stress_100, "KVDB", "GC stress: write+read+delete >=100 cycles") {
    ...
    trace_event(&g_trace, "  [GC-STRESS] start: keys=K00..K%02d target_gc=%u seed=0x%08X", GC_STRESS_KEYS - 1,
        GC_STRESS_TARGET, g_kv_sz_prng);
    ...
    while (g_db.stats.gc_runs < GC_STRESS_TARGET && loops < GC_STRESS_MAX_LOOPS) {
        ...
        loops++;
        if ((loops % 100) == 0) {
            trace_event(&g_trace, "  [GC-STRESS] loop=%u set=%u get=%u del=%u gc=%u", loops, n_set, n_get, n_del,
                g_db.stats.gc_runs);
        }
    }
    ...
    trace_event(&g_trace, "GC-STRESS done: set=%u get=%u del=%u loops=%u gc=%u", n_set, n_get, n_del, loops,
        g_db.stats.gc_runs);
    return 0;
}
...
TEST_CASE(kv_iter_after_gc, "KVDB", "Iterator returns latest values after GC") {
    ...
    trace_event(&g_trace, "  [KV-ITER] seeding %u keys K00..K%02d", ITER_KEY_COUNT, ITER_KEY_COUNT - 1);
    ...
    trace_event(&g_trace, "  [KV-ITER] GC-trigger loops=%u gc=%u", loops, g_db.stats.gc_runs);
    ...
    uint8_t  val_buf[16];
    uint16_t klen, vlen;
    while (rdb_kv_iter_next(&it, key_buf, sizeof(key_buf), val_buf, sizeof(val_buf), &klen, &vlen) == RDB_OK) {
        ...
        iter_count++;
        trace_event(&g_trace, "  [KV-ITER #%u] key=%s vlen=%u", iter_count, key_buf, vlen);
    }
    trace_event(&g_trace, "KV-ITER complete: %u keys found", iter_count);
    for (int i = 0; i < ITER_KEY_COUNT; i++)
        TEST_ASSERT_EQ(seen[i], 1);
    return 0;
}
...
TEST_CASE(kv_iter_busy_on_modify, "KVDB", "Iterator returns BUSY when DB modified") {
    ...
    uint16_t klen, vlen;
    TEST_ASSERT_RDB_ERR(
        rdb_kv_iter_next(&it, key_buf, sizeof(key_buf), val_buf, sizeof(val_buf), &klen, &vlen), RDB_ERR_BUSY);
...
TEST_CASE(kv_corrupt_sector_recovery, "KVDB", "Corrupt sector header recovery") {
    ...
    trace_event(&g_trace, "  [KV-CORRUPT] wrote key=%s val_len=%u", key, (uint16_t)sizeof(val));
...
TEST_CASE(kv_mixed_value_stress, "KVDB", "Uniform random mixed-length value GC stress (full-range coverage)") {
    ...
    for (int i = 0; i < MIX_KV_KEY_COUNT; i++) {
        ...
        if (key_max_vals[i] < min_max_val)
            min_max_val = key_max_vals[i];
    }
    trace_event(&g_trace, "  [KV-MIX] key_max_vals range: %u..%u", min_max_val, kv_max_val_for_key(&g_db, 1));
    ...
    uint32_t loops = 0, prev_gc = g_db.stats.gc_runs;
    trace_event(&g_trace, "  [KV-MIX] start: %u keys target_gc=%u bin_w=%u", MIX_KV_KEY_COUNT, MIX_KV_GC_TARGET, bin_w);
    ...
    uint32_t covered = 0;
    trace_event(&g_trace, "Mixed-KV decile histogram (writes=%u bin_w=%u):", total_writes, bin_w);
    for (uint32_t b = 0; b < HIST_BINS; b++) {
        ...
        if (hist[b] > 0u)
            covered++;
        trace_event(&g_trace, "  bin[%u] %5u..%-5u: %u writes (%.1f%%)", b, lo, hi, hist[b],
            total_writes > 0 ? (double)hist[b] * 100.0 / (double)total_writes : 0.0);
    }
    trace_event(&g_trace,
        "Mixed-KV coverage: %u/%u deciles hit (need >=%u) "
        "min=%u max=%u loops=%u gc=%u",
        covered, HIST_BINS, HIST_MIN_COVERAGE, min_size, max_size, loops, g_db.stats.gc_runs);
...
int main(void) {
    test_config_t config = {.log_file = fopen(test_make_log_path("kvdb_stress"), "w"),
        .verbose                      = 1,
        .stop_on_fail                 = 0,
        .filter                       = NULL,
        .post_test_hook               = post_test_kvdb_sectors,
        .hook_ctx                     = NULL};
