...
int sim_flash_init(
    sim_flash_t* f, uint8_t* buf, uint32_t size, uint32_t sector_size, uint32_t page_size, uint8_t write_gran) {
    if (!f || !buf || size == 0 || sector_size == 0)
        return -1;
...
int sim_flash_read(sim_flash_t* f, uint32_t addr, uint8_t* buf, size_t len) {
    ...
    if (f->trace)
        trace_flash_read(f->trace, addr, buf, len);
...
int sim_flash_write(sim_flash_t* f, uint32_t addr, const uint8_t* buf, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (f->fault_ctx && fault_should_power_loss(f->fault_ctx, addr, (uint32_t)i)) {
            if (f->trace)
                trace_flash_write(f->trace, addr, before, f->mem + addr, trace_len);
            return -1;
        }
    }
