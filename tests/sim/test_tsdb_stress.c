...
TEST_CASE(ts_rotation_stress, "TSDB", "Rotation stress: append+query >=100") {
    ...
    trace_event(&g_trace, "  [ROT-STRESS] start: target=%u seed=0x%08X", ROT_TARGET, g_ts_sz_prng);
    ...
    while (g_db.stats.sector_rotations < ROT_TARGET && loops < ROT_MAX_LOOPS) {
        ...
        if ((n_append % 200) == 0 && time > 100u) {
            ts_wg_ctx_t qctx = {0};
            TEST_ASSERT_RDB_OK(trace_ts_query(&g_db, time - 100u, time - 1u, ts_wg_query_cb, &qctx));
        ...
        if (time % 500 == 0)
            trace_event(&g_trace, "  [TS-ROT] time=%u append=%u query=%u rot=%u", time - 1, n_append, n_query,
                g_db.stats.sector_rotations);
    }
    TEST_ASSERT_GE(g_db.stats.sector_rotations, ROT_TARGET);
    TEST_ASSERT_GT(n_query, 0u);
    trace_event(&g_trace, "TS-ROT done: append=%u query=%u latest=%u loops=%u rot=%u", n_append, n_query, n_latest,
        loops, g_db.stats.sector_rotations);
    return 0;
}
...
static int find_last_record(uint32_t* out_addr, rdb_ts_record_hdr_t* out_hdr) {
    ...
    while (off + sizeof(rdb_ts_record_hdr_t) <= end) {
        ...
        if (rh.magic == 0xFFu && rh.state == 0xFFu)
            break;
        if (rh.magic != RDB_TS_RECORD_MAGIC || rh.data_len == 0 || rh.data_len > g_db.max_data_len) {
            off += (uint16_t)RDB_ALIGN_UP(sizeof(rdb_ts_record_hdr_t), g);
            continue;
        }
        uint32_t rsz = (uint32_t)sizeof(rdb_ts_record_hdr_t) + RDB_ALIGN_UP(rh.data_len, g);
        if (off + rsz > end)
            break;
        *out_addr = base + off;
        *out_hdr  = rh;
...
static int ts_crc_cb(uint32_t t, const void* data, uint16_t len, void* arg) {
    ...
    if (!data) {
        ctx->null_count++;
        TEST_ASSERT_EQ(len, ctx->expect_len);
    }
    return RDB_ITER_CONTINUE;
}
...
TEST_CASE(ts_crc_corruption, "TSDB", "CRC corruption reported in query/get_latest") {
    ...
    uint16_t out_len  = 0;
    uint32_t out_time = 0;
    TEST_ASSERT_RDB_ERR(rdb_tsdb_get_latest(&g_db, &out_time, out, sizeof(out), &out_len), RDB_ERR_CRC);
    ...
    trace_event(&g_trace, "  [TS-CRC] query ts=1..%u: total=%u null=%u", CRC_RECORDS, qctx.total, qctx.null_count);
    return 0;
}
...
static int corrupt_sealed_header(void) {
    for (uint8_t s = 0; s < g_db.sector_cnt; s++) {
        ...
        if (h.magic != RDB_TS_SECTOR_MAGIC)
            continue;
        if (h.count == 0xFFFFu || h.end_off == 0xFFFFu)
            continue;
...
TEST_CASE(ts_degraded_active_recovery, "TSDB", "Recover from degraded ACTIVE sector") {
    ...
    trace_event(&g_trace, "  [TS-DEGRADED] wrote ts=1..%u rotations=%u count=%u", time - 1, g_db.stats.sector_rotations,
        g_db.total_count);
    ...
    trace_event(&g_trace, "  [TS-DEGRADED] recovered: oldest=%u newest=%u count=%u", oldest, newest, g_db.total_count);
    return 0;
}
...
static uint16_t ts_rand_size(uint32_t max) {
    ...
    if (r < 40u) {
        ...
        if (max > 1025u)
            sz = 1025u + (ts_xorshift() % (max - 1025u));
        else
            sz = max;
    } else {
        sz = max;
...
static int ts_mix_verify_cb(uint32_t t, const void* data, uint16_t len, void* arg) {
    ...
    if (c->count < 3 || c->count % 20 == 0)
        trace_event(&g_trace, "  [TS-QUERY #%u] time=%u len=%u", c->count, t, len);
    c->count++;
    return RDB_ITER_CONTINUE;
}
...
TEST_CASE(ts_mixed_payload_stress, "TSDB", "Random mixed-length payload rotation stress (1..max)") {
    ...
    while (g_db.stats.sector_rotations < MIX_TS_ROT_TARGET && loops < MIX_TS_MAX_LOOPS) {
        ...
        if (sz <= 32)
            ...
            size_bins[4]++;
        if (sz >= 1000) {
            trace_event(&g_trace, "  [LARGE-TS #%u] payload_len=%u time=%u rot=%u", large_writes, sz, time,
                g_db.stats.sector_rotations);
            large_writes++;
        }
    ...
    trace_event(&g_trace,
        "Mixed-TS size distribution (records=%u large=%u): "
        "1..32=%u 33..256=%u 257..1024=%u 1025..%u=%u max=%u",
        time - 1, large_writes, size_bins[0], size_bins[1], size_bins[2], max_dl - 1u, size_bins[3], size_bins[4]);
    trace_event(&g_trace, "Mixed-TS max_val=%uB loops=%u rotations=%u", max_size, loops, g_db.stats.sector_rotations);
    ...
    trace_event(&g_trace, "  [TS-QUERY] range ts=%u..%u (db_count=%u)", oldest, newest, rdb_tsdb_count(&g_db));

    ts_mix_verify_t vfy = {0, 0, 0, 0, 0, max_dl};
    TEST_ASSERT_RDB_OK(rdb_tsdb_query(&g_db, oldest, newest, ts_mix_verify_cb, &vfy));
    TEST_ASSERT_EQ(vfy.count, rdb_tsdb_count(&g_db));
    TEST_ASSERT_EQ(vfy.oversize, 0u);
    TEST_ASSERT_EQ(vfy.stamp_mismatches, 0u);
    trace_event(&g_trace, "  [TS-QUERY] result: count=%u stamp_mismatches=%u", vfy.count, vfy.stamp_mismatches);
...
int main(void) {
    test_config_t config = {.log_file = fopen(test_make_log_path("tsdb_stress"), "w"),
        .verbose                      = 1,
        .stop_on_fail                 = 0,
        .filter                       = NULL,
