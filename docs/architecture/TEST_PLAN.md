...
| 新特性测试 | KVDB Cache + TSDB 安全修复 | 缓存读写/热键更新/GC迁移/碰撞压力/32B max-key cache hit、TSDB 追加查询环形/安全恢复/初始化格式、多变长数据分布(5类:1-255B) | 正确性验证 + 压力测试 + 碰撞压力 |

每个测试单元编译为独立可执行文件，互不依赖，可以并行运行。
全量入口在 8 个基础测试完成后会执行 `rdbdump_offline_verify`，用模拟 Flash dump 反向验证离线解析工具。
...
- **KVDB 最大记录长度**: 由 `data_cap - record_header - key_overhead` 决定，随 key 长度变化；大 value 分块写入时每块长度仍按 `write_gran` 对齐。
- **TSDB 最大数据长度**: 由 `data_cap - record_header` 决定，固定值；当前仅支持 `write_gran=0/1`。
...
**数据分布设计**:
- Key 长度: 50% 短键(1-8B) / 30% 中键(9-24B) / 15% 长键(25-31B) / 5% 最大键(32B)。
...
**方法**:
- 固定测试矩阵：KVDB 对 1/2/4/8B 写入粒度重复运行，并覆盖非对齐尾部的大 value；TSDB 当前对支持的 1/2B 粒度重复运行，4/8B 受扇区头 2B 提交字段限制，初始化和格式化必须返回 `RDB_ERR_PARAM`。
- Cache/边界矩阵：`test_kvdb_cache` 在脚本和 CMake 中使用 `RDB_KV_CACHE_SIZE=64`，覆盖 32 字节 max-key cache hit 校验。
- De-dup 矩阵：构造相同 hash、长度和 8 字节前缀但完整 key 不同的记录，重新 init 后两个 key 均必须可读。
- Epoch 查询矩阵：先写入高时间戳旧 epoch，再 reset epoch 写入低时间戳，范围查询不得因旧 epoch 超上界而提前停止。
...
- 所有测试单元从项目根目录编译，输出到 `tests/out` 统一目录。
- 构建前自动清理旧的中间文件（`.ilk`、`.pdb`、`.o`），确保干净编译。
- `build` 目录下的批处理入口、根 `Makefile`、`tests/perf/Makefile` 和 CMake 的可执行文件/静态库/PDB 输出均配置到 `tests/out`；CMake 的 `CMakeFiles`、`CMakeCache.txt` 等生成系统元数据仍保留在构建目录中。
- Windows 默认优先使用 LLVM/Clang，已安装路径为 `D:\Programs\LLVM`；当 Clang 不可用时回退到 GCC，已安装路径为 `D:\Programs\w64devkit`。
- 批处理脚本会自动检测 `D:\Programs\LLVM\bin\clang.exe`，再检测 `D:\Programs\w64devkit\bin\gcc.exe`。
- 编译选项统一为 C99，并启用 `-Wall -Wextra -std=c99 -D_CRT_SECURE_NO_WARNINGS`。
- 提供批量运行脚本，可选择性运行 KVDB 套件、TSDB 套件或全部测试。
- 全量测试在 8 个基础套件运行完成后，使用模拟 Flash dump 执行 `rdbdump` 离线校验和导出，导出目录为 `tests/out/rdbdump_export/<YYMMDD-HHMMSS>/`。
...
| 功能域 | KVDB 基础 | KVDB 压力 | KVDB Cache | TSDB 基础 | TSDB 压力 | 集成 | 故障注入 | 示例 | rdbdump |
|--------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| 初始化/格式化 | ✓ |   | ✓ |   |   |   |   | ✓ | ✓ |
| 写入/读取 | ✓ | ✓ | ✓ |   |   |   |   | ✓ | ✓ |
| 更新/覆盖 | ✓ | ✓ | ✓ |   |   |   |   |   | ✓ |
...
| 断言总数 | ~48,000（随构建配置变化） |
| 通过率 | 100% |
| 单次全量运行耗时 | ~150 ms（-O2，不含编译） |
| CRUD 覆盖 | 全测试套件覆盖 SET/GET/DELETE/APPEND/QUERY |
