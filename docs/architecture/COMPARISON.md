...
| 模块 | RocketDB (v1.6.0) | FlashDB 2.2.0 | LittleFS 2.11.3 |
|------|-------------------|---------------|-----------------|
| 公共头文件 | 1,388 行 | — | — |
| KVDB 引擎 | 3,176 行 / ~24 KB (x86) / ~12 KB (ARM估) | ~4,584 B (obj) | — |
| TSDB 引擎 | 2,120 行 / ~13.5 KB (x86) / ~7 KB (ARM估) | ~1,160 B (obj) | — |
| 核心/工具 | — | ~694 B (obj) | — |
| **总计** | **~6,684 行 / ~37.5 KB (x86) / ~19 KB (ARM估)** | **~6.4 KB (obj)** | **~13-17 KB** |

> RocketDB v1.6.0 代码行数约 6,700 行，x86-64 .text ~37.5 KB，估 ARM Thumb2 ~19 KB。FlashDB 最轻量 (6.4 KB)，LittleFS 居中。

### 2.2 RAM 占用

| 组件 | RocketDB | FlashDB | LittleFS |
|------|----------|---------|----------|
| 数据库句柄 (KVDB) | 87–1,108 B (默认 87 B) | ~120 B | N/A |
| 数据库句柄 (TSDB) | 88 B + 4N | ~80 B | N/A |
| 每扇区元数据 (RAM) | 48 B (16+32 bloom) | ~128 B | N/A |
| 键缓存 (每槽) | 16 B (默认关闭) | 8-12 B (可配置) | N/A |
| 去重表 (stack) | 256 B (16 slots, 可配置) | — | N/A |
| 静态全局 RAM | ~2 B | **2 B** | < 2 KB |
| 运行时栈 | 200–800 B | 200–500 B | ~4 KB (文件缓冲) |
| **典型配置** | **~1,200-2,650 B** | **~200-500 B** | **~2-6 KB** |
| **最小配置** | **~637 B** (CACHE=0,BLOOM=0) | — | — |

> **RocketDB v1.6.0 默认关闭 KV cache**，KVDB 句柄 87 字节。启用 `-DRDB_KV_CACHE_SIZE=64 -DRDB_BLOOM_BITS=256` 恢复 v1.5.x 行为 (1,108 B)。最小配置 ~637 B RAM。
