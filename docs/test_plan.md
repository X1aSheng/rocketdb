...
| 新特性测试 | KVDB Cache + TSDB 安全修复 | 缓存读写/热键更新/GC迁移/碰撞压力/长 key cache hit、TSDB 追加查询环形/安全恢复/初始化格式、多变长数据分布(5类:1-255B) | 正确性验证 + 压力测试 + 碰撞压力 |
...
- Cache/边界矩阵：`test_kvdb_cache` 在脚本和 CMake 中使用 `RDB_KV_CACHE_SIZE=64`、`RDB_MAX_KEY_LEN=254`，覆盖长 key cache hit 校验和 254 字节 key 配置。
