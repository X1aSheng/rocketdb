...
4. `strkey_len()` used 8-bit loop state in the former extended-key design. The
   current architecture now limits KVDB keys to 32 bytes, and the size_t scan is
   retained as defensive code.
...
| KVDB cache max key verification | Added max-key cache-hit coverage with `RDB_KV_CACHE_SIZE=64` and the 32-byte architecture limit. | Cache-hit key verification uses `RDB_MAX_KEY_LEN` storage. | Batch and CTest cache suite passed. |
...
| KVDB max key length handling | Max-key boundary tests now target the 32-byte architecture limit. | `strkey_len()` uses `size_t` scan state and bounded `uint8_t` assignment. | Batch and CTest passed with max-key configuration. |
