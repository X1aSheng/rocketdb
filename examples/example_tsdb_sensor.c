...
static rdb_err_t open_clean_tsdb(rdb_tsdb_t* db, const rdb_partition_t* part, uint32_t* erase_counts) {
...
int main(void) {
    ...
    for (i = 0u; i < SAMPLE_COUNT; ++i) {
        ...
        if (require_ok(rdb_tsdb_append(&db, timestamp, &sample, sizeof(sample)), "append sample") != 0) {
            return 1;
        }
    }

    if (require_ok(rdb_tsdb_query(&db, base_time + 300u, base_time + 900u, collect_sample, &summary), "query samples")
        != 0) {
        return 1;
    }
    ...
    out_len = 0u;
    if (require_ok(rdb_tsdb_get_oldest(&db, &oldest_time, &oldest_sample, sizeof(oldest_sample), &out_len), "get "
                                                                                                            "oldest")
        != 0) {
        return 1;
    }
    ...
    out_len = 0u;
    if (require_ok(rdb_tsdb_get_latest(&db, &latest_time, &latest_sample, sizeof(latest_sample), &out_len), "get "
                                                                                                            "latest")
        != 0) {
        return 1;
    }
    if (out_len != sizeof(latest_sample) || latest_time != base_time + ((SAMPLE_COUNT - 1u) * 60u)
    ...
    if (range_oldest != oldest_time || range_newest != latest_time || rdb_tsdb_count(&db) != SAMPLE_COUNT) {
    ...
    printf("  stats: writes=%u reads=%u rotations=%u lost=%u\n", (unsigned)stats.write_ops, (unsigned)stats.read_ops,
        (unsigned)stats.sector_rotations, (unsigned)stats.records_lost);
