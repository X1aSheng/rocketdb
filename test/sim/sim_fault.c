...
{
    if (!ctx || rule_id >= ctx->rule_count) return;
 ...
 * @brief 检查规则是否应该触发
 */
static int should_trigger_rule(fault_ctx_t *ctx, const fault_rule_t *rule,
                               uint32_t op_count, uint32_t addr, size_t len)
{
...
int fault_should_read_fail(fault_ctx_t *ctx, uint32_t addr, size_t len)
{
    ...
    for (uint32_t i = 0; i < ctx->rule_count; i++) {
        ...
        if (rule->type != FAULT_TYPE_READ_FAIL) continue;
...
int fault_should_write_fail(fault_ctx_t *ctx, uint32_t addr, size_t len)
{
    ...
    for (uint32_t i = 0; i < ctx->rule_count; i++) {
        ...
        if (rule->type != FAULT_TYPE_WRITE_FAIL && 
            rule->type != FAULT_TYPE_POWER_LOSS) continue;
...
int fault_should_erase_fail(fault_ctx_t *ctx, uint32_t addr)
{
    ...
    for (uint32_t i = 0; i < ctx->rule_count; i++) {
        fault_rule_t *rule = &ctx->rules[i];
        if (rule->type != FAULT_TYPE_ERASE_FAIL) continue;
...
int fault_should_power_loss(fault_ctx_t *ctx, uint32_t addr, uint32_t offset)
{
    ...
    if (!ctx || !ctx->power_loss_armed) return 0;
    
    if (ctx->write_count == ctx->power_loss_at_write && 
        offset >= ctx->power_loss_at_byte) {
        ctx->fault_injected++;
        ctx->fault_by_type[FAULT_TYPE_POWER_LOSS]++;
                             ...
                             uint32_t addr, uint32_t len)
{
    ...
    for (uint32_t i = 0; i < ctx->rule_count; i++) {
        ...
        if (rule->type != FAULT_TYPE_DATA_CORRUPT) continue;
...
void fault_quick_region_fail(fault_ctx_t *ctx, uint32_t addr_start, 
                             uint32_t addr_end, uint32_t pct)
{
    if (pct > 100) pct = 100;
...
int fault_export_rules(fault_ctx_t *ctx, char *buf, size_t len)
{
    ...
    for (uint32_t i = 0; i < ctx->rule_count; i++) {
        ...
        offset += snprintf(buf + offset, len - offset,
                          "%d,%d,%u,%u,%u,%u,%u,%d\n",
                          r->type, r->trigger_mode, r->trigger_count,
                          r->probability_pct, r->addr_start, r->addr_end,
                          r->seed, r->enabled);
        if ((size_t)offset >= len - 1) break;
    }
